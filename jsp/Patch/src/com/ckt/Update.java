...
public class Update {
	...
	public static void update(PatchDeployment pd) 
	{
		Connection conn = null;
		PreparedStatement stmt = null;
		String sql = "UPDATE patch_deployment SET idpatch=?, project=?, internal_bug_id=? WHERE idpatch_deployment = ?";
		try {
			conn = DbConnect.getInstance().getConnection();
			stmt = conn.prepareStatement(sql);
			
			stmt.setString(1, ""+pd.getIdpatch());
			stmt.setString(2, ""+pd.getProject());
			stmt.setString(3, ""+pd.getInternal_bug_id());
			stmt.setString(4, ""+pd.getIdpatch_deployment());
			
			stmt.executeUpdate();
		} catch (Exception e)
		{
			e.printStackTrace();
			System.out.println("err: function delete(void)");
		}
		finally
		{
	...
	public static int delete(String index)
	{
		...
		PreparedStatement stmt = null;
		int err = -1;
		String sql = "DELETE FROM patch_deployment WHERE idpatch_deployment = ?";
		try {
			conn = DbConnect.getInstance().getConnection();
			stmt = conn.prepareStatement(sql);
	...
	public static int insert(String sql, PatchDeployment pd)
	{
		Connection conn = null;
		PreparedStatement stmt = null;
		int err = -1;
	...
	public static int insert(String sql, PatchInfo pi)
	{
		Connection conn = null;
		PreparedStatement stmt = null;
		int err = -1;
